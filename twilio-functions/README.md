...
**Functions Service stood up 2026-09-13.** Service `coloradosprings-deck-call-routing` (domain `coloradosprings-deck-call-routing-7183.twil.io`) created; all six functions above added with exact content and Public visibility, each live-verified via a browser GET (correct TwiML, no "Unauthorized"). `@sendgrid/mail@8.1.4` added as a dependency (initially missed on first Deploy All, caught by a live GET on `/send-voicemail-email` returning a module-not-found error; fixed and redeployed). `SITE_LABEL`, `ALERT_EMAIL_TO`, `ALERT_EMAIL_FROM` set to this site's values. Deploy All succeeded, confirmed live at the domain above.

**Greeting uploaded 2026-09-13.** The Chair recorded his own voice greeting; converted from the handed-over m4a to `greeting.mp3` (mono, 44.1kHz, 15.15s) and uploaded to the Functions Service as a Public asset via the Chrome extension's file-input upload (the console's native file-picker click can't be driven by browser automation directly). Deployed, then independently live-verified with a cache-busted `fetch()` in a fresh tab: 200, `audio/mpeg`, 191,966 bytes, exact match to the local file, not just the console's own "Asset created" log line (finding #39 on this same console logged a case where that log line lied).

**Still Chair-only, not done:** buy the real local number and set its Voice webhook to `/incoming-call` (8-10 real 719-area-code options already presented to the Chair, awaiting his pick+purchase); set `OWNER_NUMBER` (and optional `PARTNER_NUMBER`); generate a SendGrid API key and paste it into `SENDGRID_API_KEY` (until then, `/send-voicemail-email` and `/voicemail-done`'s email step will fail with a SendGrid auth error, by design, not a defect); the two-phone live test. Once the number is provisioned, `src/lib/site.js`'s `phoneDisplay`/`phoneHref` still need to be swapped from the `(719) 555-0100` placeholder to the real number, and pushed.
